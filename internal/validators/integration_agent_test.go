...
func TestIntegratorAgentReferencesConsistencyTools(t *testing.T) {
	...
	if !strings.Contains(content, `"check_consistency"`) && !strings.Contains(content, `check_consistency(`) && !strings.Contains(content, "`check_consistency`") {
		t.Error("integrator agent missing reference to check_consistency")
	}
	if !strings.Contains(content, `"process_consistency_gate"`) && !strings.Contains(content, `process_consistency_gate(`) && !strings.Contains(content, "`process_consistency_gate`") {
		t.Error("integrator agent missing reference to process_consistency_gate")
	}
	if !strings.Contains(content, `"validate_canon"`) && !strings.Contains(content, `validate_canon(`) && !strings.Contains(content, "`validate_canon`") {
