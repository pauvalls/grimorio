...
Before running this skill, the architect MUST have already invoked
`grimorio-consolidator` (delegate) which leaves the campaign in a state
where `verify_campaign_freshness` reports `campaign_md_stale=false` (or the
stale files are about to be regenerated by the integrator). If
`consolidate_campaign` is still showing open `AmbiguityQuestion`s, escalate
to the user and DO NOT proceed with PDF assembly.
